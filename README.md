...
This is the standard for Astra, a language designed for astronomy and astronautics simulation, modeling and visualization.
...
## 1. Script Layout and block types.
Any Astra script file is divided into blocks; there are three block types: 
+ `space`
+ `observer`
+ `output`
...
End
```
Block type `observer` defines coordinate reference frame, camera settings, and observer movements. For instance:
...
Block type `output` defines output product properties. For example, the next code is used to get a 3D Scene as output.
```
Start output scene
...
In some cases components have proper names. For example the constant `Moon` is equivalent to `Earth moon`.
...
Functions are defined outside of the block types. With the next syntactic structure:

```
<object_type> Function_name (<parameters>)
